...
iqac_monitor_metrics_bp = Blueprint('iqac_monitor_metrics', __name__)

@iqac_monitor_metrics_bp.route('/iqac_monitor_metrics')
def iqac_monitor_metrics():
    ...
    dbsession = DBSession()
    assigned_metrics = dbsession.query(Metric_Assign).all() # Fetch all assigned metrics
    
    # Close the database session
    dbsession.close()

    # Render the template with the fetched assigned metrics
    return render_template('iqac/iqac_monitor_metric.html', assigned_metrics=assigned_metrics)

@iqac_monitor_metrics_bp.route('/iqac_monitor_metric_download/<string:file_id>/<path:filename>')
def iqac_monitor_metric_download(file_id, filename):
    dbsession = DBSession()
...
@iqac_monitor_metrics_bp.route('/iqac_monitor_metric_view/<string:file_id>/<path:filename>')
def iqac_monitor_metric_view(file_id, filename):
    dbsession = DBSession()
    metric_details = dbsession.query(Metric_Assign).filter(Metric_Assign.id == file_id).first()
    dbsession.close()  # Remember to close the session
    filenames = metric_details.metric_pdf.split(',')
...
@iqac_monitor_metrics_bp.route('/iqac_monitor_metric_delete/<string:file_id>/<filename>')
def iqac_monitor_metric_delete(file_id, filename):
    dbsession = DBSession()
    metric_details = dbsession.query(Metric_Assign).filter(Metric_Assign.id == file_id).first()
    filenames = metric_details.metric_pdf.split(',')
...
@iqac_monitor_metrics_bp.route('/iqac_monitor_metric_mail/<string:file_id>')
def iqac_monitor_metric_mail(file_id):    
    dbsession = DBSession()
    metric_details = dbsession.query(Metric_Assign).filter(Metric_Assign.id == file_id).first()
    ...
    return render_template('iqac/dashboard.html',message='Mail Sent Sucessfuly')
